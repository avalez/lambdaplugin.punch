...
module.exports = {
    ...
    endpoint: "http://api.bitbucket.org/1.0/repositories/azhdanov/jiratimesheet/wiki/",
    ...
    getContent: function(path, callback) {
        ...
        request.get({url: self.endpoint + '/' + page, json: true}, function (err, response, body) {
            if (!err && response.statusCode == 200) {
		        ...
		        var collected_contents = {site_title: 'JIRA Timesheet Plugin', title: title, contents: parsed_output, wiki: true};
    ...
    getContentPaths: function(basepath, callback) {
        default_content_handler.getContentPaths(basepath, function(dummy, collected_paths) {
	      if (basepath.match(/^\/$/)) {
            collected_paths.push('/wiki/Overview');
            collected_paths.push('/wiki/Frequent_Questions');
            collected_paths.push('/wiki/Plugin_Configuration');
            collected_paths.push('/wiki/Tips_and_Tricks');
            collected_paths.push('/wiki/Email_Time_Sheet_Report');
            collected_paths.push('/wiki/Timesheet_Holidays');
            collected_paths.push('/wiki/Multiple_Time_Zones');
            collected_paths.push('/wiki/RESTful_endpoint');
            collected_paths.push('/wiki/Unified_Plugin_Views');
            collected_paths.push('/wiki/Commercial');
   	      }
